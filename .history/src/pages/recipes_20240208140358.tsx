...
const RecipesPage = () => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const response = await axios.get(`https://api.edamam.com/search?q=${searchQuery}&app_id=${APP_ID}&app_key=${apiKey}&to=12`);
...
const RecipesPage = () => {
  ...
  const handleSearch = async (query: string) => {
    ...
    const recipeURL = process.env.NEXT_PUBLIC_EDAMAM_API_URL;
    try {
      const response = await axios.get(`${recipeURL}?q=${query}&app_id=${APP_ID}&app_key=${apiKey}&to=12`);
      setRecipes(response.data.hits);
    } catch (error) {
      console.error('Error searching recipes:', error);
    }
